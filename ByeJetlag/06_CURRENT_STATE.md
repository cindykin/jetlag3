...
- [ ] **BUG BARU DITEMUKAN (2026-09-11):** `CircadianEngine.generateBlocks` tidak menerapkan `adaptationRate` (0.95/1.53 jam/hari, `04_ALGORITHM.md` Section 6/7) pada hari-hari **recovery** (pasca-arrival). Baris `completedShiftDays = min(max(dayOffset + preparationDays, 0), preparationDays)` membuat shift **beku** di nilai hari terakhir preparation untuk SEMUA `dayOffset >= 0` — sleep time jadi identik persis setiap hari recovery, tidak progresif menutup `Remaining Gap` seperti diminta spec ("Estimasi Hari Sembuh = ceil(Remaining Gap / Adaptation Rate)"). **Akibat konkret (ditelusuri manual, angka contoh Jakarta→Paris arah westward):** karena sleep window sama persis tiap malam, window `seekLight` hari berikutnya (`[CBTmin-4h, CBTmin]`) jatuh **seluruhnya di dalam** window Sleep malam sebelumnya → dihapus total oleh `enforcingSleepExclusivity`. `avoidLight` tersisa cuma potongan ~30 menit. Ini kemungkinan besar penyebab konkret keluhan "sepanjang hari cuma keliatan Sleep doang" pada hari-hari recovery — BUKAN cuma efek visual dari bug split/grouping yang sudah diperbaiki di atas. `caffeine`/`noCaffeine`/`nap` tetap muncul normal (tidak overlap Sleep), jadi gejalanya bukan "section kosong total", tapi "section timpang, minus seekLight/avoidLight tiap hari recovery". Perlu fix terpisah di `CircadianEngine.swift`: hitung shift progresif untuk `dayOffset >= 0` pakai `adaptationRate`, bukan cuma freeze di `preparationDays`. **STATUS: MASIH TERBUKA** — lihat entry di bawah (2026-09-13) yang menutup pertanyaan berbeda (apakah MEKANISME subtraction-nya sendiri benar), bukan bug freeze ini.
- [x] **Investigated, confirmed correct behavior per spec — no code change (2026-09-13):** Pertanyaan sempit yang berbeda dari bug freeze di atas: "apakah wajar Seek Light ter-subtract SELURUHNYA oleh Sleep untuk arah westward, karena CBTmin (wake - 2.5j) masih berada di tengah window tidur?" — **JAWABAN: YA, ini perilaku benar sesuai rumus**, bukan bug. Dikonfirmasi terhadap `04_ALGORITHM.md` Section 3.B & 3.C (sudah direvisi): Section 3.C poin 1 eksplisit bilang "Sleep eksklusif mutlak... TIDAK ADA block lain yang boleh render bersamaan", dan poin 2 bilang overlap dengan Avoid Light "Sleep yang menang, dipotong/disesuaikan". Kalau kalkulasi CBTmin menaruh window Seek Light/Avoid Light seluruhnya di dalam window Sleep, hasil "block itu hilang total" adalah konsekuensi LANGSUNG dari aturan overlap yang memang diminta spec — bukan efek samping algoritma yang salah. **Catatan penting: ini TIDAK menutup bug freeze `completedShiftDays` di atas** — freeze itu tetap bug terpisah yang bikin kondisi "CBTmin di tengah Sleep" ini terjadi BERULANG setiap hari recovery (harusnya cuma kadang-kadang terjadi tergantung progres shift harian, bukan konstan). Task ini murni dokumentasi — **tidak ada perubahan kode** di `CircadianEngine.swift`/`enforcingSleepExclusivity` sesuai instruksi.
...
- [x] `ScheduleView` memiliki timezone divider per hari: divider hanya muncul untuk arrival ke timezone baru (badge `{Kota} Time` + ikon jam) atau midnight lokal tanpa perpindahan timezone (tanpa badge). Top bar navigation tidak berubah. *(2026-09-11)*
- [x] Day-grouping, hour-grid axis, dan section header `ScheduleView` sekarang dievaluasi memakai timezone airport yang aktif di titik waktu tersebut (origin sebelum departure, destination setelah arrival) via `activeTimeZone(at:flights:)` — sebelumnya jatuh ke `Calendar.current` (device timezone), yang bisa menggeser posisi grid & label tanggal/jam kalau device tidak sejajar dengan timezone trip. *(2026-09-11)*
- [x] **Rewrite (bukan tambal):** Segmentasi timeline sekarang dihitung dari "boundary instants" (`computeBoundaries` di `Services/ScheduleTimelineBuilder.swift`) — midnight lokal & flight arrival ke timezone baru — dihitung independen dari block manapun, BUKAN dari pengelompokan `block.startTime` per hari seperti sebelumnya. Block yang melintasi boundary (mis. Sleep yang melewati tengah malam) sekarang di-split jadi beberapa `PositionedBlock` (potongan visual, id unik per potongan, tetap reference `sourceBlockID` ke block asli untuk tap-to-detail). Fungsi-fungsi ini dipindah dari `private` di `ScheduleView.swift` ke `Services/ScheduleTimelineBuilder.swift` (internal) supaya bisa di-unit-test — lihat `Tests/ScheduleTimelineBuilderTests.swift`. *(2026-09-11)*
- [x] **Resolved as expected behavior, disambiguation ditambahkan (bukan bug fix):** Investigasi debug print (2026-09-12) mengkonfirmasi `buildSections()` sudah benar secara kronologis. Yang sempat kelihatan seperti bug: saat pesawat melintasi tengah malam DI TIMEZONE ASAL lalu tak lama kemudian mendarat dan melintasi tengah malam lagi DI TIMEZONE TUJUAN, dua `SectionHeader` berurutan bisa menampilkan teks jam identik (mis. "00:00") padahal beda beberapa jam absolut — karena masing-masing memang benar diformat di timezone yang berbeda (`boundary.timeZone`nya beda), bukan salah hitung. Fix: `DaySection` sekarang punya `timeZoneLabel` (nama kota singkat dari `boundary.timeZone`, via `shortTimeZoneLabel(for:)`), ditampilkan di `SectionHeader` biasa sebagai "00:00 · Jakarta" / "00:00 · Paris" — bukan cuma di `TimezoneDividerView` yang khusus arrival. *(2026-09-12)*
- [x] **Bug fix:** `TimelineGrid.hourTicks` label jam sekarang dihitung dari `Date` asli (`DaySection.segmentStartDate` + `timeZone`, field baru), bukan `hour % 24` dari angka relatif terhadap `section.startHour`. Sebelumnya section yang mulai dari jam non-bulat (mis. arrival 03:01) tetap nampilin "00:00" di baris pertama grid, padahal jam sebenarnya 03:00-an. Posisi visual (`offset`) block/grid-line TIDAK berubah — itu memang sudah benar sebagai aritmatika relatif, cuma teks labelnya yang salah. *(2026-09-12)*
- [x] **Konsolidasi header:** `TimezoneDividerView` (bar terpisah: tanggal + badge kota + jam) dan `SectionHeader` (bar terpisah: tanggal + jam) yang tadinya dirender berurutan (2 bar bertumpuk untuk section boundary) sekarang digabung jadi SATU `SectionHeader` yang baca `section.divider` langsung — badge kota cuma muncul kalau `divider?.cityName != nil` (arrival timezone baru), border bawah cuma muncul kalau `divider != nil` (midnight ATAU arrival, bukan section pertama trip). `TimezoneDividerView` sudah dihapus dari `ScheduleView.swift`. *(2026-09-12)*
- [ ] **BELUM DIVERIFIKASI VISUAL:** Task ini awalnya minta screenshot simulator trip Jakarta-Doha sebagai bukti sebelum lapor selesai — **ini TIDAK bisa dilakukan dari sesi ini** karena environment yang dipakai untuk generate/edit kode ini tidak punya Xcode/simulator/macOS sama sekali (Linux sandbox, sama seperti keterbatasan yang sudah dicatat di entry-entry sebelumnya soal `xcodebuild`). Kode sudah di-review manual (source review, tidak dijalankan) dan konsisten secara struktur, tapi verifikasi visual asli (spacing badge, alignment 2-baris jadi 1-baris, posisi tick jam yang benar) WAJIB dilakukan manual di Xcode/simulator sebelum item ini ditandai selesai beneran.
- [x] **Fitur baru: Trim timeline ke "sekarang" (2026-09-13).** Section yang seluruh rentangnya sudah lewat (`segmentEndDate <= Date()`) tidak ditampilkan sama sekali (`visibleSections` di `ScheduleView`, bukan cuma disembunyikan sebagian). Section yang sedang berjalan saat ini (mengandung `Date()`) di-trim: grid mulai render dari jam sekarang dibulatkan ke jam sebelumnya (`displayStartHour`), bukan dari `section.startHour` asli — dihitung di timezone segmen itu sendiri (bukan device). Section header TETAP menampilkan tanggal/jam asli section (tidak ikut ke-trim, cuma area grid-nya). Section hari mendatang render penuh seperti biasa, tidak terpengaruh. Fitur display-only, tidak ada di `04_ALGORITHM.md` sebagai requirement algoritma — murni UX supaya orang buka app siang hari nggak perlu scroll lewatin jam-jam yang udah kelewat.
